...
const Content = () => {
  ...
  const { isLoading, data } = useQuery("getAllQuestions", () => {
    if (topic) {
      return newRequests
        .get(`http://3.104.172.87:8080/find/${topic}`)
        .then((res) => res.data);
    } else {
      return newRequests
        .get("http://3.104.172.87:8080/questions")
        .then((res) => res.data);
    }
  });
      ...
      {data.length > 0 &&
        data.map((question, index) => {
          return (
            ...
            >
              <div className="w-full bg-white p-4 md:p-5 rounded-lg shadow-md flex items-start gap-5 relative">
                <div className="grid grid-cols-4 hidden md:block absolute right-2 top-2">
                  {question.tags.map((tag, index) => {
                    return <Badge className="mx-1" content={tag} />;
                  })}
                </div>
